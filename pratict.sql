...
CREATE TABLE employ (
    employed_id SERIAL PRIMARY KEY,
    employee_name VARCHAR(25) NOT NULL,
    department_id INTEGER REFERENCES departments (department_id),
    salary DECIMAL(10, 2),
    hire_date DATE
);
INSERT INTO employ(employee_name, department_id, salary, hire_date) VALUES
...
-- i went to use nested query
SELECT employee_name FROM employ
WHERE department_id IN (
    SELECT department_id FROM departments WHERE department_id = 6
);
SELECT * FROM employ
JOIN departments ON employ.department_id = departments.department_id


SELECT departments.department_name, ROUND(avg(employ.salary), 2) as average_salary
FROM employ
JOIN departments USING(department_id)
GROUP BY department_name;

-- count employ
SELECT departments.department_name, count(employed_id)
FROM employ
JOIN departments USING(department_id)
GROUP BY department_name;

-- height employ name and avg salary
SELECT department_name, round(avg(salary)) as avg_salary FROM employ 
JOIN departments USING(department_id)
GROUP BY department_name
ORDER BY avg_salary DESC
LIMIT 1

-- how to get data and year

SELECT * FROM employ

select extract(YEAR from hire_date) as hire_year, count(*) from employ
GROUP BY hire_year;
